...
if [ -z "$3" ]; then
	if [ "$NEW" == '0' ]; then
		...
		do
			...
			#/usr/bin/sudo -u $USER
			make clean GLUON_TARGET=$TARGET GLUON_BRANCH=$BRANCH GLUON_AUTOUPDATER_BRANCH=$BRANCH GLUON_AUTOUPDATER_ENABLED=1 GLUON_RELEASE=$MY_RELEASE BROKEN=$BROKEN -j $THREADS
		done
	fi
fi
...
echo "> make update"
#/usr/bin/sudo -u $USER
make update GLUON_BRANCH=$BRANCH GLUON_AUTOUPDATER_BRANCH=$BRANCH GLUON_AUTOUPDATER_ENABLED=1 GLUON_RELEASE=$MY_RELEASE BROKEN=$BROKEN -j $THREADS
...
do
	...
	date
	#/usr/bin/sudo -u $USER
	make GLUON_TARGET=$TARGET GLUON_BRANCH=$BRANCH GLUON_AUTOUPDATER_BRANCH=$BRANCH GLUON_AUTOUPDATER_ENABLED=1 GLUON_RELEASE=$MY_RELEASE BROKEN=$BROKEN -j $THREADS $DEBUG
done
...
if [ -d "$BASE_DIR/$BRANCH/gluon/output/images" ]; then
	...
	#/usr/bin/sudo -u $USER
	make manifest GLUON_BRANCH=$BRANCH GLUON_AUTOUPDATER_BRANCH=$BRANCH GLUON_AUTOUPDATER_ENABLED=1 GLUON_RELEASE=$MY_RELEASE BROKEN=$BROKEN
